...
class FindZodiacFragment : Fragment() {
    ...
    private val viewModel: FindZodiacViewModel by lazy {
        val db = ZodiacDb.getInstance(requireContext())
        val factory = FindZodiacViewModelFactory(db.dao)
        ViewModelProvider(this, factory)[FindZodiacViewModel::class.java]
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewModel.getNavigation().observe(viewLifecycleOwner, {
            ...
            findNavController().navigate(
                FindZodiacFragmentDirections.actionFindZodiacFragmentToDetailsFragment(
                    it
                )
            )
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        when (item.itemId) {
            R.id.menu_history -> {
                findNavController().navigate(
                    R.id.action_findZodiacFragment_to_historyFragment
                )
                return true
            }
            R.id.menu_about -> {
